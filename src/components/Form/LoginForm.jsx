...
const LoginForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (email === "test@example.com" && password === "password") {
        ...
        setError("Credenciales inválidas. Intenta con test@example.com / password");
      }

    } catch (err) {
      // Manejo de errores desde la API
      const errorMessage = err.response?.data?.message || 'Ha ocurrido un error inesperado. Por favor, inténtalo de nuevo.';
  ...
  return (
    <div className="relative bg-[var(--color-surface)] dark:bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl shadow-[var(--shadow-dental-xl)] w-full max-w-6xl overflow-hidden md:flex md:min-h-[600px]">
      ...
      >
        <img src="/Login.png" alt="Fondo de inicio de sesión" className="absolute inset-0 w-full h-full object-cover" />
      ...
      >
        ...
        <AnimatePresence>
          {isSuccess && <Alert key="success-alert" type="success" title="¡Bienvenido!" message="Has iniciado sesión correctamente. Redirigiendo..." />}
          {error && <Alert key="error-alert" type="error" title="Error de autenticación" message={error} />}
        </AnimatePresence>
        ...
        <Form onSubmit={handleSubmit} className="space-y-6">
          ...
          <p className="mt-2 text-center text-[var(--color-text-secondary)] text-sm">
            <Link href="/solicitar-reset" className="font-semibold text-primary hover:text-accent underline">
              ¿Olvidaste tu contraseña?
            </Link>
          </p>
          <p className="mt-2 text-center text-[var(--color-text-secondary)] text-sm">
            ¿No tienes una cuenta?{' '}
            <Link href="/register" className="font-semibold text-primary hover:text-accent underline">
              Regístrate aquí
            </Link>
          </p>
        </Form>
